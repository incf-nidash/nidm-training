...
function processUrl(URL) {
  ...
  $.ajax({
    ...
    // Duplicate code to below, hacky for now, can be streamlined!
    store.load("text/turtle", buffer,
          function(success, results) {
			  ...
			  var tstat_url = "http://" + window.location.hostname + ":" + window.location.port + "/nifti-drop-master/" + baseurl + "/" + filename;
			  console.log(tstat_url);
			  results[i]["location"]["value"] = tstat_url;
		      ...
		      //hack a URL
		     //cut out nidm.ttl part and replace it with filename from T-stat query
