<?php
 ...
 // $terms = get_conte_terms_hash("yes");
  ...
  $arr = get_rat_dti_data_within_slice($scantype, $hemi, $slice, $comp, $std);
  $data = $arr['data'];
  $query = $arr['query'];
...
<?php
  ...
  if ($download == 'yes'){
    $time = time();
    $filename = "rat-dti-".lcfirst($scantype)."-".lcfirst($hemi)."-".$slice;
    if ($comp != '*'){
      $filename .= "-".$comp."-".$std;
    }
    $filename .= "-".$time.".csv";
  ...
  echo "Rat fragmentation data for scan type:<strong>" . $scantype . "</strong> hemisphere:<strong>" . $hemi;
  echo "</strong> slice number:<strong>" . $slice . "</strong> ";
  if ($comp != '*'){
